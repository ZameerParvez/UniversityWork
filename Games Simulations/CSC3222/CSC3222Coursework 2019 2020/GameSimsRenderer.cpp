...
void GameSimsRenderer::RenderFrame()	{
	...
	Matrix4 ortho = Matrix4::Orthographic(-1.0f, 1.0f, tileSize * (float)numTilesX, 0.0f, 0.0f, tileSize * (float)numTilesY); //each sprite is 16 pixels, times how many sprites we want

	glUniformMatrix4fv(projMatrix, 1, false, ortho.array);

	glUniform2f(screenSize, tileSize * (float)numTilesX, tileSize * (float)numTilesY); //
...
void GameSimsRenderer::DrawCircle(Vector2 worldPos, float radius, Vector4 colour) {
	...
	Vector3 currentVert = Vector3((float)sin(0), (float)cos(0), 0) * radius;
	Vector3 worldStart	= Vector3(worldPos.x, worldPos.y, 0);
